...
namespace algorithm::rbtree {
	...
	private:
		...
		auto rb_min(node<K, V>* current) const -> decltype(this->root); // find the minimum node from the subtree of current
		...
		void rb_delete_all_nodes(node<K,V>* current);
