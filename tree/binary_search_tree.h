...
namespace algorithm::tree {
	...
	public:
		...
		void print_tree() const { print_tree(root, 0); } // debug
	...
	private: // hepler functions
		...
		void print_tree(bst_node<K, V>* current, int num) const;
	...
	template<typename K, typename V>
	void BinarySearchTree<K, V>::print_tree(bst_node<K, V>* current, int num) const {
		if(current != nullptr){
			print_tree(current->left, num * 2 + 1);
			std::cout << "key: " << current->key << " node num: " << num << std::endl;
			print_tree(current->right, num * 2 + 2);
		}
	}
